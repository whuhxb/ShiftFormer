...
"""
Based on model1, change to DW local
...
class TokenMixer(nn.Module):
    ...
    def forward(self, x):
        _,_,W,H = x.size()
        small = self.smallconv(x)
        big = self.bigconv(x)

        small = F.interpolate(small, (W,H))
        big = F.interpolate(big, (W,H))
...
@register_model
def model12_s12(pretrained=False, **kwargs):
    """
