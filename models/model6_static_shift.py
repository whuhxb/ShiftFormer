...
"""
Based on model5, add orginal shift
...
class Shifting(nn.Module):
    ...
    def forward(self, x):
        ...
        out[:, g * 4:g * 5, :, :-1] = x[:, g * 4:g * 5, :, 1:]  # shift left
        out[:, g * 5:g * 6, :, 1:] = x[:, g * 5:g * 6, :, :-1]  # shift right
        out[:, g * 6:g * 7, :-1, :] = x[:, g * 6:g * 7, 1:, :]  # shift up
        out[:, g * 7:g * 8, 1:, :] = x[:, g * 7:g * 8, :-1, :]  # shift down
...
@register_model
def model6_static_shiftformer_s12(pretrained=False, **kwargs):
    """
...
if __name__ == '__main__':
    ...
    model = model6_static_shiftformer_s12()
    out = model(input)
    print(model)
    print(out.shape)
