...
class TokenMixer(nn.Module):
    def __init__(self, dim, act_layer=nn.GELU, params=params):
        ...
        if params["spatial_mixer"]["fc_factor"]>1:
            self.fc1 = nn.Sequential(
                nn.Linear(dim, max(dim//params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"])),
                self.act,
                nn.Linear(max(dim//params["spatial_mixer"]["fc_factor"], params["spatial_mixer"]["fc_min_value"]), dim)
            )
        else:
    ...
    def forward(self, x):
        if hasattr(self,"gc1"):
            gc1 = self.gc1(x)
            x = x + gc1
        x = self.act(self.fc1(self.dw1(x.contiguous()).permute(0,2,3,1).contiguous())).permute(0,3,1,2).contiguous()
        x = x.contiguous()
        ...
        if hasattr(self, "fc2"):
            if hasattr(self, "gc2"):
                gc2 = self.gc2(x)
                x = x + gc2
            x = self.act(self.fc2(self.dw2(x.contiguous()).permute(0,2,3,1).contiguous())).permute(0,3,1,2).contiguous()
        x = x.contiguous()
...
@register_model
def fcvt_v6_64_B12(pretrained=False, **kwargs):
...
@register_model
def fcvt_v6_64_B24(pretrained=False, **kwargs):
    fcvt_params = params.copy()
...
@register_model
def fcvt_v6_64_B48(pretrained=False, **kwargs):
    fcvt_params = params.copy()
