...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0, 
        ...
        self.proj1 = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size,
                              stride=stride, padding=padding)
        self.proj2 = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size,
                              stride=stride, padding=padding)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def forward(self, x):
        x = self.proj1(x)+ self.proj2(x)
        x = self.norm(x)
        return x
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, 
        ...
        self.fc1_1 = nn.Conv2d(in_features, hidden_features, 1)
        self.fc1_2 = nn.Conv2d(in_features, hidden_features, 1)
        self.act = act_layer()
        self.fc2_1 = nn.Conv2d(hidden_features, out_features, 1)
        self.fc2_2 = nn.Conv2d(hidden_features, out_features, 1)
    ...
    def forward(self, x):
        x = self.fc1_1(x)+self.fc1_2(x)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2_1(x) + self.fc2_2(x)
        x = self.drop(x)
        return x
...
@register_model
def poolformer_pd_s12(pretrained=False, **kwargs):
    """
