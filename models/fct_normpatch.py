...
"""
Based on model13_full_conv_transformer, replace the patchEmbed. layerNorm before conv.
...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, patch_size=7, stride=4, in_chans=3, embed_dim=768, norm_first=True):
        super().__init__()
...
class BaseFormer(nn.Module):
    def __init__(self, layers, embed_dims=None,
        ...
        self.patch_embed = OverlapPatchEmbed(patch_size=7, stride=4, in_chans=3, embed_dim=embed_dims[0], norm_first=False)
...
@register_model
def fct_s12_64_7118_TTT_normpatch(pretrained=False, **kwargs):
    layers = [2, 2, 6, 2]
...
if __name__ == '__main__':
    ...
    model = fct_s12_64_7118_TTT_normpatch()
    out = model(input)
    print(model)
    print(out.shape)
