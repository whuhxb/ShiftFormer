#based on fcvt v2, fc replace conv1x1 for speed.
import os
import math
import copy
import torch
...
class GlobalContext(nn.Module):
    def __init__(self, dim, act_layer=nn.GELU, params=params):
        ...
        self.fc = nn.Sequential(
            nn.Linear(dim, dim//params["spatial_mixer"]["globalcontext_reduction"]),
            act_layer(),
            nn.Linear(dim//params["spatial_mixer"]["globalcontext_reduction"], dim)
        )


    def forward(self,x):
        gc = self.gap(x).squeeze(dim=-1).squeeze(dim=-1)
        gc = self.fc(gc).unsqueeze(dim=-1).unsqueeze(dim=-1)
        return gc  # [b,c,1,1]
...
@register_model
def fcvt_v3_s12_64_TFFF(pretrained=False, **kwargs):
...
@register_model
def fcvt_v3_s12_64_debug(pretrained=False, **kwargs):
    ...
    fcvt_params["spatial_mixer"]["globalcontext_reduction"]=16
    layers = [2, 2, 6, 3]
    embed_dims = [64, 128, 320, 512]
    mlp_ratios = [4, 4, 3, 3]
    downsamples = [True, True, True, True]
