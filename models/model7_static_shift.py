...
"""
Based on model1, exchange the shifting operation to the mid of MLPs.
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None,
                 out_features=None, act_layer=nn.GELU, drop=0., n_groups=12):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1)
        self.act = act_layer()
        self.shift = Shifting(n_groups=n_groups)
...
class PoolFormerBlock(nn.Module):
    ...
    def __init__(self, dim, n_groups=12, mlp_ratio=4.,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                       act_layer=act_layer, drop=drop, n_groups=n_groups)
...
@register_model
def model7_static_shiftformer_s12(pretrained=False, **kwargs):
    """
...
@register_model
def model7_static_shiftformer_s24(pretrained=False, **kwargs):
    """
    PoolFormer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
...
@register_model
def model7_static_shiftformer_s12_n16(pretrained=False, **kwargs):
...
@register_model
def model7_static_shiftformer_s12_n8(pretrained=False, **kwargs):
...
if __name__ == '__main__':
    ...
    model = model7_static_shiftformer_s12_n16()
    out = model(input)
    print(model)
    print(out.shape)
