...
class Shifting(nn.Module):
    ...
    def forward(self, x):
        ...
        out[:, g * 4:g * 5, :, :-3] = x[:, g * 4:g * 5, :, 3:]  # shift left *3
        out[:, g * 5:g * 6, :, 3:] = x[:, g * 5:g * 6, :, :-3]  # shift right *3
        out[:, g * 6:g * 7, :-3, :] = x[:, g * 6:g * 7, 3:, :]  # shift up *3
        out[:, g * 7:g * 8, 3:, :] = x[:, g * 7:g * 8, :-3, :]  # shift down *3
...
@register_model
def model4_static_shiftformer_s12(pretrained=False, **kwargs):
    """
...
if __name__ == '__main__':
    ...
    model = model2_static_shiftformer_s12()
    out = model(input)
    print(model)
    print(out.shape)
