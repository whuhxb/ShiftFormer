...
"""
Based on model1, change PatchEmbedding to overlapped Embedding for comparison
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        self.patch_embed = OverlapPatchEmbed(img_size=img_size, patch_size=7 ,
                                             stride=4, in_chans=3, embed_dim=embed_dims[0])
        ...
        for i in range(len(layers)):
            ...
            if downsamples[i] or embed_dims[i] != embed_dims[i+1]:
                ...
                network.append(
                    ...
                    #     )
                    OverlapPatchEmbed(img_size= img_size // (2 ** (i + 1)),
                        patch_size= 3,
                        stride= 2,
...
@register_model
def model10_static_shiftformer_s12(pretrained=False, **kwargs):
    """
...
@register_model
def model10_static_shiftformer_s12_8844(pretrained=False, **kwargs):
    """
    PoolFormer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
...
@register_model
def model10_static_shiftformer_s24_8844(pretrained=False, **kwargs):
    """
    PoolFormer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
...
if __name__ == '__main__':
    ...
    model = model10_static_shiftformer_s12()
    out = model(input)
    print(model)
    print(out.shape)
