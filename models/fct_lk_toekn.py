...
class TokenMixer(nn.Module):
    def __init__(self, dim, act_layer=nn.GELU, s_att_ks=7, s_att_r=4, useBN=True, useSpatialAtt=True):
        ...
        self.dw3x3 = nn.Conv2d(dim, dim, kernel_size=5, padding=2, stride=1, groups=dim)
...
@register_model
def fct_s12_64_7118_TTT_lk_token(pretrained=False, **kwargs):
    layers = [2, 2, 6, 2]
...
if __name__ == '__main__':
    ...
    model = fct_s12_64_7118_TTT_lk_token()
    out = model(input)
    print(model)
    print(out.shape)
